...
class StackVM(object):
	...
	def vm_start(self):
		...
		self.vm.launch_vm_process(None, 'gui', '')
		
		print 'waiting vm start ...'
		while True:
			
			with self.vm.create_session() as session:
				try:
	...
	def get_ip(self):
		...
		if len(self.vm.enumerate_guest_properties('/VirtualBox/GuestInfo/Net/0/V4/IP')[1]) < 1:
			time.sleep(0.2)
		try:
			self.vmip = self.vm.enumerate_guest_properties('/VirtualBox/GuestInfo/Net/0/V4/IP')[1][0]
			return self.vmip
	...
	def vm_export(self):
		...
		self.vm.export_to(appliance, self.vm_name)
	...
	def vm_import(self):
		try:
			...
			raise e + ' please make sure the imagefile("' + IMAGE_PATH + '") is exist;'
